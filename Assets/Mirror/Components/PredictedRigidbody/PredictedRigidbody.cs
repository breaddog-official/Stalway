    ...
    public class PredictedRigidbody : NetworkBehaviour
    {
        ...
        void ApplyState(double timestamp, Vector3 position, Quaternion rotation, Vector3 velocity, Vector3 angularVelocity)
        {
...
#endif
            {
                ...
                if (syncPosition) predictedRigidbody.position = position;
                if (syncRotation) predictedRigidbody.rotation = rotation;
            ...
            else if (mode == PredictionMode.Fast)
            {
                ...
                if (syncPosition) predictedRigidbody.MovePosition(position);
                if (syncRotation) predictedRigidbody.MoveRotation(rotation);
            }
        ...
        void OnReceivedState(double timestamp, RigidbodyState state)//, bool sleeping)
        {
            ...
            double posThreshold = isOwned ? positionCorrectionThresholdSqr : 0.0;
            double rotThreshold = isOwned ? rotationCorrectionThreshold : 0.0;
            ...
            if (compareLastFirst &&
                ...
                positionToStateDistanceSqr < posThreshold &&                               // fast comparison
                Quaternion.Angle(state.rotation, physicsRotation) < rotThreshold)
            {
            ...
            if (newest.timestamp < state.timestamp)
            {
                ...
                if (positionToStateDistanceSqr >= posThreshold) // fast comparison
                {
            ...
            if (positionToInterpolatedDistanceSqr >= posThreshold || // fast comparison
                                                                     //positionToInterpolatedDistance >= positionCorrectionThreshold ||     // slow comparison
                rotationToInterpolatedDistance >= rotThreshold)
            {
        ...
        public override void OnDeserialize(NetworkReader reader, bool initialState)
        {
            ...
            Quaternion rotation = Quaternion.identity;//data.rotation;
