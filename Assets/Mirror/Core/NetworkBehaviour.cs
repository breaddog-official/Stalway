...
namespace Mirror
{
    ...
    public abstract class NetworkBehaviour : SerializedMonoBehaviour
    {
        ...
        {
            var scriptClass = this.GetType();

            // check for all SyncVar fields, they don't have to be visible
            foreach (FieldInfo field in GetAllFields(scriptClass, typeof(NetworkBehaviour)))
            {
                if (IsSyncVar(field))
                {
                    return true;
        ...
        public static bool IsSyncVar(FieldInfo field)
        {
