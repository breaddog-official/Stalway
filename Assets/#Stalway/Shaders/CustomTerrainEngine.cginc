// Upgrade NOTE: upgraded instancing buffer 'UnityTerrain' to new syntax.
...
inline float4 Squash(in float4 pos)
{
    ...
    float4 planeNormal = UNITY_ACCESS_INSTANCED_PROP(UnityTerrain, _SquashPlaneNormal);
    ...
    // optimized version:
    float3 projectedVertex = pos.xyz - (dot(planeNormal.xyz, pos.xyz) + planeNormal.w) * planeNormal.xyz;

    pos = float4(lerp(projectedVertex, pos.xyz, UNITY_ACCESS_INSTANCED_PROP(UnityTerrain, _SquashAmount)), 1);
...
{
    pos.xyz *= UNITY_ACCESS_INSTANCED_PROP(UnityTerrain, _TreeInstanceScale).xyz;
    float3 bent = mul(UNITY_ACCESS_INSTANCED_PROP(UnityTerrain, _TerrainEngineBendTree), float4(pos.xyz, 0.0)).xyz;
